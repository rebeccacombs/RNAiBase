  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await axios.get(
        `${BASE_URL}esearch.fcgi?db=pubmed&term=${query}&retmax=${num}&retmode=json&api_key=${api_key}`
      );
...
export async function fetchData(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const response = await axios.get(
        `${BASE_URL}efetch.fcgi?db=pubmed&id=${id_list}&retmode=xml&api_key=${api_key}`
      );
    ...
    const pData = data.PubmedArticleSet.PubmedArticle.map((article: any) => {
      try {
        ...
        return {
          ...
          abstract:
            article.MedlineCitation.Article.Abstract.AbstractText._ ||
            dataTools.getAbstractText(article.MedlineCitation.Article.Abstract.AbstractText),
          authors: authorList ? dataTools.getAuthors(authorList) : [],
          journal: article.MedlineCitation.Article.Journal.Title._,
          pubdate: new Date(
            dataTools.getDate(article.MedlineCitation.Article.Journal.JournalIssue.PubDate)
          ),
        ...
        console.error(
          ...
          article.MedlineCitation.PMID._,
...
const dataTools = {
  ...
  getSlug(title: any): string {
    //entry = data.PubmedArticleSet.PubmedArticle[IDX].MedlineCitation.Article.ArticleTitle._
  ...
  getAbstractText(entry: any): string {
    //entry = data.PubmedArticleSet.PubmedArticle[IDX].MedlineCitation.Article.Abstract.AbtractText
